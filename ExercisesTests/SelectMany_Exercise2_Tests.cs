...
namespace ExercisesTests
{
    ...
    public class SelectMany_Exercise2_Tests
    {
        ...
        public void ShallBuildBestMarksAndStudentsCorrectly()
        {
            ...
            var expectedResult = new[] { "Lucy: 6", "Lucy: 6", "Alice: 5", "Tom: 5", "Tom: 4" };

            CollectionAssert.AreEqual(
                expectedResult,
                result,
                $"For students {EnumerableToString(students)} the result shall be {EnumerableToString(expectedResult)} but it was {EnumerableToString(result)}"
            );
        }
    }
}
