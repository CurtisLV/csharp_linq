...
namespace ExercisesTests
{
    ...
    public class Where_QuerySyntax_RefactoringChallenge_Tests
    {
        ...
        {
            var words = new[] { "racoon", "raven", "lion", "crane", "sardine" };

            var expectedResult = new[] { "crane", "racoon", "raven", };
            ...
            CollectionAssert.AreEqual(
                expectedResult,
                result,
                $"For INPUT {EnumerableToString(words)} the RESULT shall be '{EnumerableToString(expectedResult)}' BUT IT WAS '{EnumerableToString(result)}'"
            );
        }
        ...
        {
            var words = new[] { "trout", "heron", "rhinoceros", "duck", "tiger" };

            var expectedResult = new[] { "heron", "rhinoceros", "trout", };
