    ...
    public class OfType_RefactoringChallenge_Tests
    {
        ...
        public void DateTimeIsPresent()
        {
            ...
            Assert.AreEqual(
                expectedResult,
                result,
                $"Expected result for input (null, '2020/1/10', 1, 'THERE', 2020/1/15, 2020/1/5) is {expectedResult.ToString("yyyy/MM/dd")}, but was {resultAsString}"
            );
        }
        ...
        {
            var input = new object[] { null, 1, "THERE" };

            var result = OfType.GetTheLatestDate(input);
            var resultAsString = result.HasValue ? result.Value.ToString("yyyy/MM/dd") : "null";
            Assert.Null(
                result,
                $"Expected result for input (null,  1, 'THERE') is null, but was {resultAsString}"
            );
        }
        ...
        public void EmptyCollection()
        {
            ...
            Assert.Null(
                result,
                $"Expected result for empty collection is null, but was {resultAsString}"
            );
        }
