        ...
        public static class QuerySyntax
        {
            ...
            public static void Run()
            {
                ...
                var petsSelectedByUserLighterThan5Kilos =
                    from petIndexPair in Data.Pets.Select(
                        (pet, index) => new { Pet = pet, Index = index }
                    )
                    where
                        indexesSelectedByUser.Contains(petIndexPair.Index)
                        && petIndexPair.Pet.Weight < 5
                ...
                var countOfAllDogsHeavierThan30KilosA = (
                    from pet in Data.Pets
                    where pet.PetType == PetType.Dog && pet.Weight > 30
                    select pet
                ).Count();

                Printer.Print(
                    countOfAllDogsHeavierThan30KilosA,
