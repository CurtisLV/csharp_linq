...
namespace LinqTutorial.MethodSyntax
{
    static class IntersectExcept
    {
        ...
        public static void Run()
        {
            ...
            Printer.Print(
                petsIntersectDefaultEqualityCheck,
                nameof(petsIntersectDefaultEqualityCheck)
            );
            ...
            var alice = new PetOwner(
                1,
                "Alice",
                new[] { Data.Pets.ElementAt(0), Data.Pets.ElementAt(2), Data.Pets.ElementAt(3) }
            );
            var bob = new PetOwner(
                2,
                "Bob",
                new[] { Data.Pets.ElementAt(0), Data.Pets.ElementAt(1), Data.Pets.ElementAt(2) }
            );

            //let's find those Pets who are "shared" between Bob and Alice
            var sharedBetweenBobAndAlice = alice.Pets.Intersect(
                bob.Pets,
                new PetEqualityByIdComparer()
            );
            ...
            var petsWithOneOwnerOnly = bob.Pets
                .Concat(alice.Pets) //those are all pets
